...
- [ ] Manually dispatch the dashboard publication workflow and inspect its jobs, Release assets, tag target, deployed Pages data tree, and absence of regular schedule. The current `gh` token receives HTTP 403 for workflow dispatch and requires user authorization or a manual web dispatch before this live-only step can continue.
