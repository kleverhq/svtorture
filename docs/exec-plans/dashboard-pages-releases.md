...
- [x] (2026-08-01 03:00Z) Run final local gates: `just smoke` passed 118 focused Python and 81 frontend tests; `just unit` passed 185 deterministic Python tests; `just ci` passed pre-commit, lint, metadata, unit, frontend build/tests, 11 Docker tests, and the real Icarus smoke run. Schema regeneration was clean, deterministic/archive tests passed, and a generated v6 site served valid index/trends resources over local HTTP.
- [x] (2026-08-01 03:10Z) Complete the prompt-to-artifact audit. It mapped all code requirements to artifacts and found one integration defect: the frontend-generated replay command used CLI-style flags against a positional `just reproduce` recipe. Corrected it to the canonical four positional arguments and strengthened the exact-command frontend test; no implementation omissions remain before live GitHub validation.
- [x] (2026-08-01 03:40Z) Push the implementation to `main` and inspect the first enabled CI run. The run exposed ANSI styling in `GITHUB_ACTIONS` splitting option names in a help-text assertion; changed the test to strip ANSI with the standard library and reproduced the pass under the Actions environment locally.
- [x] (2026-08-01 08:45Z) Inspect the first manual publication run `30691978041`. Policy and Pages publication completed, but `just --quiet ci-matrix` suppressed the JSON matrix, so no collect jobs were created and the run failed after publishing an all-missing campaign. Fixed the root cause by making the recipe command silent with `@`, capturing normal `just ci-matrix` stdout, and ensuring a command failure fails the policy step; focused review found and fixed the initial command-substitution error masking, and the control review is clean.
- [x] (2026-08-01 12:35Z) Complete live GitHub validation. Run `30694850482` attempt 3 passed policy, all three collection jobs, immutable Release publication, bounded Pages upload, and Pages deployment from commit `96ebc44`; CI run `30692512111` is also green. The one-time GHCR setup made the three exact image digests anonymously pullable. Three Release summaries reconstruct trends, every Release has exactly the ZIP and summary assets with matching SHA-256 and tag targets, and distinct reruns produced distinct immutable campaign IDs; exact same-ID no-op and collision behavior remains covered by mocked API tests. Pages serves only the timestamp-latest campaign, older detail URLs and `dataset.json` return 404, no `gh-pages` branch exists, headless Chrome rendered all three tool results, remote manifest and Release ZIP replay validation passed, and the workflow remains manual-only with no schedule.
...
Milestone 3 is implemented, reviewed, and live-validated. Mocked GitHub tests cover first publication, exact no-op retry, interrupted draft completion, tag and asset collisions, backfill ordering, unchanged summary history, and latest-only Pages. Workflow source tests prove enabled CI, manual/main-only publication, least-privilege job permissions, full action-SHA pinning, short artifact retention, and absence of `gh-pages`/clobber operations. Live run `30694850482` attempt 3 collected all publication tools, published two-asset campaign Releases, and deployed latest-only Pages. The final smoke gate reports 118 focused Python and 81 frontend tests; `just unit` reports 185 deterministic Python tests passed.
