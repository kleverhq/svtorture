...
- [x] (2026-08-10 01:41Z) Committed Milestone 2 as `9292648`.
- [x] (2026-08-10 02:03Z) Implemented Milestone 3 VPI mechanics and both cases. Verilator and VCS conform on callback timing and packed-structure size; Icarus conforms on callback timing and records a normal nonconforming packed-structure result.
- [x] (2026-08-10 02:24Z) Completed Milestone 3 correctness, standards, architecture, and mandatory KISS/YAGNI reviews. Removed fixture-generated aborts, strengthened callback ordering across occupied and empty queues, simplified generated startup sources/work files, and aligned documentation. Fresh correctness control reported `No substantive findings`; ponytail reported `Lean already. Ship.` Final `just smoke` passed with 163 backend, 17 annotator, and 110 dashboard tests.
- [ ] Commit Milestone 3.
...
- Observation: Icarus itself provides `iverilog-vpi`, but the project image omitted a C++ compiler and make. Adding those runtime packages makes the callback case conform; its packed-structure query does not produce the expected handle/size and is normal nonconforming execution evidence rather than structural lack of VPI. The earlier apparent crash came from the fixture's own abort and was removed during review.
  Evidence: exact-image campaign `20260809T231613Z-b18485debe129a84`.

- Observation: Current Verilator supports startup callbacks and object access but aborts on `vpi_register_systf`; `--bbox-sys` permits the source-level system-task call but does not provide runtime registration.
  Evidence: exact-image failures under Verilator 5.051 followed by successful adapter-generated `cbStartOfSimulation` registration in campaign `20260809T230242Z-8d1b9ae111052432`.
...
- Decision: Make each VPI fixture invoke one standard user-defined system task and export one common calltf routine. Icarus registers the task through an adapter-generated startup source; VCS generates a `/work` PLI table mapping the task to calltf and links directly with `-debug_access+all`; Verilator uses an adapter-generated `cbStartOfSimulation` wrapper to invoke the same calltf because its current VPI implementation does not support `vpi_register_systf`.
  Rationale: Case sources remain simulator-neutral and free of private visibility pragmas or tool conditionals. VCS retains its documented `-P` path, while adapter-owned registration, fallback startup, loading, and visibility let each capable tool execute the same requirement-checking calltf.
...
Milestones 0, 1, and 2 are committed as `d3a4d48`, `a65f018`, and `9292648` respectively. Milestone 3 is implemented and end-to-end proven, pending focused and control reviews. The catalog fingerprints and validates every declared case input, rejects undeclared files and noncanonical path aliases, and detects any resource-copy mutation before accepting evidence. Standard library maps remain the sole logical-library authority; their validated derivation survives bundles and replay. Bounded generated setup text, covergroup activation, and one generic structural unsupported reason cover the three new cases without raw tool options. SDF and configuration results match the capability matrix. The strengthened immediate-sampling oracle intentionally records current Verilator/VCS nonconformance instead of weakening the requirement.
...
Milestone 3 reuses Milestone 2 and adds only VPI-specific registration/loading and visibility behavior. Each case calls one standard user-defined system task once at time zero and exports a common calltf. The callback task proves exact-delay execution before both an occupied queue and an otherwise empty queue; the packed-structure task proves name lookup and `vpiSize` without a private source pragma. Icarus registers the task through an adapter-generated startup source; VCS maps it through an adapter-generated `/work` PLI table and enables documented debug/VPI visibility; current Verilator invokes the same calltf from an adapter-generated start-of-simulation callback because it cannot register system tasks. Capable simulators execute equivalent requirement checks; genuinely incapable profiles return structural unsupported results.
...
Plan revision note (2026-08-10 02:24Z): Recorded Milestone 3 implementation and reviews. Adapter-generated startup/loading lets all capable tools execute the same calltf; callback ordering now proves occupied and empty queues; requirement failures no longer manufacture crash evidence. Final correctness and KISS controls are clean.
